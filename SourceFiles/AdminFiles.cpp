           ...
           // 1. Fase validar si existe la carpeta padre 
            ...
            if(existe){
                MKDIR(lista_carpeta,creadas);
                    ...
                    FILE *modificar = fopen(path.c_str(), "rb+");
                    fseek(modificar,particion.part_start,SEEK_SET);
                    fread(&superbloque,sizeof(Estructuras::Superblock),1,modificar);

                    
                    // bool es = false;
                    // for(int x = 0; x < 4; x++){
                    //     if(bloque_de_carpetas.b_content[x].b_inodo == -1){
                    //         es = true;
                    //         strcpy(bloque_de_carpetas.b_content[x].b_name , padre.c_str());
                    //         bloque_de_carpetas.b_content[x].b_inodo = Inodosiguiente(superbloque,path);
                    //         fseek(modificar,superbloque.s_block_start + ( sizeof(Estructuras::Folderblock) * Bloqueactual),SEEK_SET);
                    //         fwrite(&bloque_de_carpetas,sizeof(Estructuras::Folderblock),1,modificar);
                    //     }
                    ...
                    // char bitMapInodo[superbloque.s_inodes_count];
                    // fseek(modificar,superbloque.s_bm_inode_start,SEEK_SET);
                    // fread(&bitMapInodo, sizeof(bitMapInodo), 1, modificar);
                    ...
                    //         for(int y = 0; y < 16; y++){
                    //             if(inodo.i_block[y] == -1){
                    //                  inodo.i_block[0] = Bloquesiguiente(superbloque,path); 
                    //                  break;
                    //             }
                    //         }
                    //         cout << "escribido" << x << endl;
                    //         fseek(modificar,superbloque.s_inode_start + ( sizeof(Estructuras::Inodes) * x),SEEK_SET);
                    //         fwrite(&inodo,sizeof(Estructuras::Inodes),1,modificar);
                    //         break; 
                    //     }
                          
                    // }

                    // ActualizarBitMaps(superbloque,path,"I");
                    // ActualizarBitMaps(superbloque,path,"B");
